...
export default function TikTokShopReporter() {
  ...
  const [lightboxVideo, setLightboxVideo] = useState<VideoRow | null>(null);
  ...
  useEffect(() => {
    ...
    // Update admin state when user signs in or out without page reload
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      const flag = session?.user?.user_metadata?.is_admin === true;
      setIsAdmin(flag);
      setIsLoggedIn(!!session);
      if (!session) setAdminMode(false);
      if (event === 'SIGNED_IN') { setShowLoginModal(false); load(); }
    });
    ...
    return (
      <div style={{display:"flex",background:"#fff",borderRadius:14,overflow:"hidden",
        ...
        <div style={{flexShrink:0,width:280,background:"#0a0a0a",display:"flex",flexDirection:"column"}}>
          {r.videoId ? (
            <div style={{position:"relative",flexShrink:0,width:280,height:420}}>
              ...
              <div
                onClick={() => setLightboxVideo(r)}
                title="Click to watch"
                style={{position:"absolute",inset:0,cursor:"pointer",zIndex:2,background:"transparent",display:"flex",alignItems:"flex-end",justifyContent:"flex-end",padding:8}}>
                <div style={{background:"rgba(0,0,0,0.65)",borderRadius:6,padding:"4px 9px",color:"#fff",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",gap:4,pointerEvents:"none"}}>
                  ⛶ Expand
                </div>
              </div>
            </div>
                    ...
                    <>
                      <div style={{position:"relative"}}>
                        ...
                        <div
                          ...
                          title="Click to watch"
                          style={{position:"absolute",inset:0,cursor:"pointer",zIndex:2,background:"transparent",display:"flex",alignItems:"flex-end",justifyContent:"flex-end",padding:8,borderRadius:"10px 10px 0 0"}}>
                          <div style={{background:"rgba(0,0,0,0.65)",borderRadius:6,padding:"4px 9px",color:"#fff",fontSize:11,fontWeight:600,pointerEvents:"none"}}>
                            ⛶ Expand
                          </div>
                        </div>
                      </div>
  ...
  return (
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:"#f4f5f7",minHeight:"100vh"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          ...
          {isLoggedIn ? (
            ...
            <button onClick={()=>setShowLoginModal(true)}
              style={{background:"transparent",color:"#555",border:"1px solid #333",padding:"7px 14px",borderRadius:8,cursor:"pointer",fontFamily:"inherit",fontSize:12}}>
              Admin Sign In
            </button>
          )}
        </div>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"20px"}}>
        ...
        {tab==="alltime" && (
          visAllTime.length===0
            ? <Empty msg='Upload the All-Time export from app.euka.ai/videos to get started'/>
            : <>
        ...
        {tab==="lastmonth" && (
          filteredLastMonth.length===0
            ? <Empty msg='Upload the Last Month export from app.euka.ai/videos to get started'/>
            : <>
      ...
      {lightboxVideo && (
        <div
          ...
          style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:"20px",overflowY:"auto"}}>
          <style>{`@keyframes lbfade{from{opacity:0;transform:scale(0.94)}to{opacity:1;transform:scale(1)}}`}</style>
          <div
            onClick={e => e.stopPropagation()}
            style={{position:"relative",animation:"lbfade 0.18s ease-out",display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",maxWidth:400,marginBottom:12,gap:10}}>
              <div>
                <div style={{color:"#fff",fontWeight:700,fontSize:15}}>@{lightboxVideo.creator}</div>
                <div style={{color:"#16a34a",fontWeight:600,fontSize:13}}>{f$(lightboxVideo.revenue)} · {fN(lightboxVideo.itemsSold)} sold</div>
              </div>
              <button
                onClick={() => setLightboxVideo(null)}
                style={{background:"rgba(255,255,255,0.12)",border:"1px solid rgba(255,255,255,0.2)",color:"#fff",borderRadius:8,padding:"6px 16px",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600,flexShrink:0}}>
                ✕ Close
              </button>
            </div>
            <iframe
              key={lightboxVideo.videoId}
              src={`https://www.tiktok.com/embed/v2/${lightboxVideo.videoId}`}
              style={{width:390,height:700,border:"none",borderRadius:12,flexShrink:0,display:"block"}}
              allowFullScreen allow="encrypted-media; autoplay" title={`@${lightboxVideo.creator}`}/>
            <a href={lightboxVideo.videoLink} target="_blank" rel="noopener noreferrer"
              style={{marginTop:14,display:"flex",alignItems:"center",gap:6,color:"#9ca3af",textDecoration:"none",fontSize:13}}>
              ▶ Open on TikTok ↗
            </a>
            <div style={{marginTop:8,color:"rgba(255,255,255,0.2)",fontSize:11}}>Press Esc or click outside to close</div>
          </div>
        </div>
      )}
      ...
      {showLoginModal && (
        <div
          ...
          <div onClick={e=>e.stopPropagation()} style={{position:"relative"}}>
            <button
              onClick={()=>setShowLoginModal(false)}
              style={{position:"absolute",top:-12,right:-12,width:28,height:28,borderRadius:"50%",background:"#374151",border:"none",color:"#fff",cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",zIndex:1}}>
              ✕
            </button>
            <Login />
          </div>
